...
function CartComp() {
    ...
    function handelRemoeFromCart(getCurrentItemID){
                   ...
                   <tbody className="whitespace-nowrap divide-y">
    {
        cart?.cartItems.map(item => (
            <tr key={item.id}>
                    ...
                    <div className="flex items-center gap-6 w-max">
                        ...
                        <div className="h-36 w-36 shrink-0">
                            <img
                                src={item?.thumbnail}
                                alt={item?.title}
                                className="w-full h-full object-cover"  // Ensure the image takes the full size
                            />
                        </div>
                ...
                <td className="py-5 px-4">
                    <Button onClick={()=>handelRemoeFromCart(item?.id)}>Remove</Button>
                </td>
            </tr>
        ))
    }
